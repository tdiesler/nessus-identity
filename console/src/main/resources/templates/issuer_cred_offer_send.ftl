...
<@layout.layout activeTab="issuer">
    ...
    <div class="content">
        ...
        <form action="/issuer/credential-offer/send" method="get" style="max-width: 400px;">
            <hidden name="subjectId" value="${userInfo.id}"/>
            <div class="bx--form-item-horizontal">
                <label for="subjectId" class="bx--label">Subject</label>
                <input type="text" name="subjectId" class="bx--text-input" value="${userInfo.name} - ${userInfo.email}" readonly/>
