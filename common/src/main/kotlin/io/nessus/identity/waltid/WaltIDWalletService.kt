...
import io.nessus.identity.LoginContext.Companion.AUTH_TOKEN_ATTACHMENT_KEY
...
import io.nessus.identity.types.Key
...
class WaltIDWalletService {
    ...
    suspend fun authUserInfo(token: String): WaltIdUser? {
    ...
    fun insertCredentialOffer(offerData: CredentialOfferData): String {
        val offerId = transaction {
            CredentialOfferTable.insertAndGetId {
                it[walletId] = offerData.walletId
                it[document] = offerData.document
                it[addedOn] = offerData.addedOn.toJavaInstant()
            }
        }.value
        return "$offerId"
    }
